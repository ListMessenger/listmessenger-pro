...
class UploadHandler
{
    ...
    {
        $image = (!empty($this->image_objects[$file_path])) ? $this->image_objects[$file_path] : null;
    ...
    protected function imagick_create_scaled_image($file_name, $version, $options)
    {
        ...
        // use !empty(). User might be setting max_width = 0 (auto in regular resizing). Value 0 would be considered empty when you use empty()
        if (!empty($options['max_width'])) {
            $image_resize = true;
            ...
            if (is_array($upload['tmp_name'])) {
                ...
                foreach ($upload['tmp_name'] as $index => $value) {
                    $files[] = $this->handle_file_upload(
                        ...
                        $upload['type'][$index],
                        !empty($upload['error']) ? $upload['error'][$index] : null,
                        $index,
                        $content_range
                    );
                }
    ...
    public function onUploadEnd($res)
    {
        ...
        if ($is_img) {
            if (!empty($this->options['config']['image_watermark']) && $this->options['config']['image_watermark']) {
