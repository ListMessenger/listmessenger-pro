...
if (!empty($_SESSION['RF']['language']) && file_exists('lang/'.basename($_SESSION['RF']['language']).'.php')) {
...
$info = pathinfo($path);
if (!empty($info['extension']) && !(!empty($_GET['action']) && $_GET['action'] == 'delete_folder') &&
...
if (!empty($_GET['action'])) {
    switch ($_GET['action']) {
        ...
        case 'create_file':
            ...
            if (empty($config['editable_text_file_exts']) || !is_array($config['editable_text_file_exts'])) {
        ...
        case 'paste_clipboard':
            if ((empty($_SESSION['RF']['clipboard_action']) && empty($_SESSION['RF']['clipboard']['path']))
