...
if (empty($_SESSION['RF']) || $_SESSION['RF']['verify'] != 'RESPONSIVEfilemanager') {
    exit('forbiden');
}
...
{
    if (!empty($config['ftp_host']) && $config['ftp_host']) {
        // *** Include the class
...
function create_img($imgfile, $imgthumb, $newwidth, $newheight = null, $option = 'crop', $config = [])
{
    ...
    if ($result && !empty($config['ftp_host']) && $config['ftp_host']) {
...
function create_folder($path = null, $path_thumbs = null, $ftp = null, $config = null)
{
    if ($ftp) {
        ...
        $oldumask = umask(0);
        $permission = 0755;
        if (!empty($config['folderPermission'])) {
...
function image_check_memory_usage($img, $max_breedte, $max_hoogte)
{
    if (file_exists($img)) {
        ...
        if (ini_get('memory_limit') > 0) {
            ...
            if (!empty($image_properties['bits'])) {
