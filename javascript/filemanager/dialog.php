...
if (USE_ACCESS_KEYS == true) {
    if (empty($config['access_keys']) || !in_array($_GET['akey'], $config['access_keys'])) {
        exit('Access Denied!');
    }
...
if (!empty($_GET['callback'])) {
    ...
    if (!empty($_SESSION['RF']['callback'])) {
        $callback = $_SESSION['RF']['callback'];
    }
}

$popup = !empty($_GET['popup']) ? strip_tags($_GET['popup']) : 0;
// Sanitize popup
$popup = (bool) $popup;

$crossdomain = !empty($_GET['crossdomain']) ? strip_tags($_GET['crossdomain']) : 0;
// Sanitize crossdomain
$crossdomain = (bool) $crossdomain;

// view type
if (empty($_SESSION['RF']['view_type'])) {
...
if (empty($_SESSION['RF']['descending'])) {
    $_SESSION['RF']['descending'] = true;
}

if (!empty($_GET['descending'])) {
...
$return_relative_url = !empty($_GET['relative_url']) && $_GET['relative_url'] == '1' ? true : false;
...
$field_id = !empty($_GET['field_id']) ? fix_get_params($_GET['field_id']) : null;
...
$get_params = [
    ...
    'akey' => (!empty($_GET['akey']) && $_GET['akey'] != '' ? $_GET['akey'] : 'key'),
];
...
<body>
    ...
    <input type="hidden" id="clipboard" value="<?php echo (!empty($_SESSION['RF']['clipboard']['path']) && trim($_SESSION['RF']['clipboard']['path']) != null) ? 1 : 0; ?>" />
...
?>
	...
	<div class="span12">
		...
		<?php if (!empty($config['folder_message'])) { ?>
		...
		foreach ($files as $file_array) {
		    ...
		    if ($file == '.' || (substr($file, 0, 1) == '.' && !empty($file_array['extension']) && $file_array['extension'] == fix_strtolower(trans('Type_dir'))) || (!empty($file_array['extension']) && $file_array['extension'] != fix_strtolower(trans('Type_dir'))) || ($file == '..' && $subdir == '') || in_array($file, $config['hidden_folders']) || ($filter != '' && $n_files > $config['file_number_limit_js'] && $file != '..' && stripos($file, $filter) === false)) {
		        continue;
		    }
		    ...
		    if (!empty($filePermissions[$file])) {
		        $file_prevent_rename = !empty($filePermissions[$file]['prevent_rename']) && $filePermissions[$file]['prevent_rename'];
		        $file_prevent_delete = !empty($filePermissions[$file]['prevent_delete']) && $filePermissions[$file]['prevent_delete'];
		    }
            ...
            $file_prevent_rename = false;
		            $file_prevent_delete = false;
		            if (!empty($filePermissions[$file])) {
		                if (!empty($filePermissions[$file]['prevent_duplicate']) && $filePermissions[$file]['prevent_duplicate']) {
